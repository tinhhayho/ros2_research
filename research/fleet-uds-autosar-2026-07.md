...
| 7 | Adaptive DM "primarily over DoIP", C++ API | 🟡 | per SWS_Diagnostics, DM supports **only** DoIP as transport (not CAN); API = `ara::diag`; DM also implements ASAM SOVD v1.0.0. *Re-verified 2026-07-15 against the actual SWS_Diagnostics PDFs (R22-11 §7.4 p.61 · R23-11 §7.2 p.74 · R24-11 §7.1 p.78): **DoIP-only still true in R24-11** — "Currently the Adaptive Platform only provides a detailed description of Ethernet-based network technologies, which mandates support of DoIP", with the "upcoming releases … will also detail CAN, CAN-FD, FR" sentence unchanged verbatim across all three releases; no `DoCAN` string in any PDF; only transport change in R24-11 = DoIP amendment-2023 protocol v4. The Transport Protocol API does allow plugging in custom/OEM transports. The web summary claiming "R24-11 added CAN" was search-engine hallucination.* |
